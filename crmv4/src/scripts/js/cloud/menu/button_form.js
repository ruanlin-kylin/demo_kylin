...
var btf = {
    button: {
        ...
        openTableList: function (trigger_id, params, bak, display) {
            ...
            // alert("pmStr:"+JSON.stringify(params) + pmStr2)
            if (display && display == "skipPage") {
        ...
        return str;
			...
			if(targetStr && !$.isEmptyObject(pmData)){
				if(!$.isEmptyObject(pmData.table)){
					...
					targetStr = targetStr.replace(/\\${table.(\w+)}/g,function(){return tableData[arguments[1]]});
				}
				if(!$.isEmptyObject(pmData.form)){
					var formData=pmData.form;
					targetStr = targetStr.replace(/\\${form.(\w+)}/g,function(){return formData[arguments[1]]});
				}
				if(!$.isEmptyObject(pmData.sys)){
					var sysData=pmData.sys;
					targetStr = targetStr.replace(/\\${sys.(\w+)}/g, function(){return sysData[arguments[1]]});
				}
			}
			return targetStr;
		}
		...
		jlChangBak:function(wordbook,data){		//搜索框设置级联字段
			...
			if(fieldToggleSetting && fieldToggleSetting.length>0){
				for(var i=0;i<fieldToggleSetting.length;i++){
					...
					var pmData={form:tableData,sys:sysParams};
			...
			if(uniqueCheckSetting && uniqueCheckSetting.tableListId){
				var loadMsg = layer.msg("去重校验中...",{icon:16,shade:[0.1,'#393D49'],time:60000});
...
//             '<label class="col-lg-2 control-label">' + fieldName + ("Y" == isbt ? '<font class="redRequired" color="red">*</font>' : '') + '</label>' +
//             '<div class="col-lg-10">' +
//             '<input class="form-control date-picker"   minDate="' + minDate + '" maxDate="' + maxDate + '"    ' + disabled_str + ' onkeydown="javascript:date_fun_dateDubbo(this);"  onblur="javascript:date_fun_dateBlur(this);"   ' + this.getValidateHtml(validateObj, isbt, fieldName) + ' key_name="' + fieldName + '"   key="extend_element" value="' + defaultVal + '" key_id="' + fieldKey + '" type="text" name="' + fieldKey + '" placeholder="' + (fieldName ? fieldName : "") + '"' +
//             'db_type="date" format="yyyy-MM-dd">' +
//             '</div>' +
//             '</div>';
//         return html;
...
//             '<label class="col-lg-2 control-label">' + fieldName + ("Y" == isbt ? '<font class="redRequired" color="red">*</font>' : '') + '</label>' +
//             '<div class="col-lg-10">' +
//             '<input class="form-control date-picker"  minDate="' + minDate + '" maxDate="' + maxDate + '"  ' + disabled_str + ' ' + this.getValidateHtml(validateObj, isbt, fieldName) + '  onkeydown="javascript:date_fun_dateDubbo(this);"  onblur="javascript:date_fun_dateTimeBlur(this);"   key="extend_element"  key_name="' + fieldName + '"  value="' + defaultVal + '" key_id="' + fieldKey + '" type="text" name="' + fieldKey + '" placeholder="' + (fieldName ? fieldName : "") + '"' +
//             'db_type="date" format="yyyy-MM-dd HH:mm:ss"' +
//             '</div>' +
//             '</div>';
//         return html;
...
//             '<label class="col-lg-2 control-label">' + fieldName + ("Y" == isbt ? '<font class="redRequired" color="red">*</font>' : '') + '</label>' +
//             '<div class="col-lg-10">' +
//             '<input class="form-control date-picker"  minDate="' + minDate + '" maxDate="' + maxDate + '"   ' + disabled_str + '  key="extend_element" key_name="' + fieldName + '" onkeydown="javascript:date_fun_dateDubbo(this);"  value="' + defaultVal + '" key_id="' + fieldKey + '" ' + this.getValidateHtml(validateObj, isbt, fieldName) + ' type="text" name="' + fieldKey + '" placeholder="' + (fieldName ? fieldName : "") + '"' +
//             'db_type="date" format="HH:mm:ss">' +
...
//         return html;
//     },
//     getValidateHtml: function (fieldName, fieldKey, defaultVal, fieldWidth, disabled_str, validateObj, isbt) {
//         var html = '';
//         if ("Y" == isbt) {
//             html += ' data-bv-notempty="true"';
//             html += ' data-bv-notempty-message="' + fieldName + '不能为空"';
...
//                     html += ' pattern="^[1-9]\\d{7}((0\\d)|(1[0-2]))(([0|1|2]\\d)|3[0-1])\\d{3}$|^[1-9]\\d{5}[1-9]\\d{3}((0\\d)|(1[0-2]))(([0|1|2]\\d)|3[0-1])\\d{3}([0-9]|X)$"  data-bv-regexp-message="' + fieldName + '格式不正确"';
//                 } else if (validateObj.validate == "mobile") {
//                     html += ' pattern="^(((13[0-9])|(14[0-9])|(15[0-9])|(18[0-9]))\\d{8})$"  data-bv-regexp-message="' + fieldName + '格式不正确"';
//                 } else if (validateObj.validate == "age") {
//                     html += ' min="0"  data-bv-greaterthan-inclusive="false"  data-bv-greaterthan-message="' + fieldName + '不能小于0"';
//                     html += ' max="120"  data-bv-lessthan-inclusive="false"   data-bv-lessthan-message="' + fieldName + '不能大于120"';
//                 } else if (validateObj.validate == "email") {
...
//                     html += ' pattern="^(([0-9]+)|([0-9]+\.[0-9]+))$"  data-bv-regexp-message="' + fieldName + '只能输入数字和小数"';
//                 }
//             }
//             if (validateObj.minlength || validateObj.maxlength) {
//                 html += ' data-bv-stringlength="true"';
