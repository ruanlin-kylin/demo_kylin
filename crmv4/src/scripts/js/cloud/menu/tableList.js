...
//alert(JSON.stringify(form_pm_obj));
var currentHref = window.location.href;
tableId = urlStr(currentHref, "id"); //获取菜单对象sid,也就是左侧菜单的menu_id属性
// alert(tableId);
var tableName = urlStr(currentHref, "name"); //
            ...
            if (data.result == "SUCCESS") {
                if (data.map) {
                    ...
                    if (data.map.tableTeam) {
                        ...
                        // form_db = data.map.form_db;
                        // alert("form_db:"+JSON.stringify(urlData));
                        ...
                        if (tableTeam != null) {
                            if (tableTeamType != null && tableTeamType == '0') { //表格组一般模式
                                ...
                                // $(".fuTableTeam").show();
                                // $("#yb_template").val(1);
                                // var fuTabCurrentClass = "col-lg-"+fBiLi+" col-sm-"+fBiLi+" col-xs-12";
                                // $(".fuTableTeam").addClass(fuTabCurrentClass);
                                //$(".list-content .fb_btn_panel_wrap").remove();
                    ...
                    // alert(tabId);
                    // alert(tableId+" "+tableTeamId+" "+tableTeamSid+" "+cur_menuId);
                    // alert("urlData "+JSON.stringify(urlData));
                    // if (urlData) {
                    //     for (ud in urlData) {
                    //         console.log("form_db:" + form_db);
                    //         if (!form_db[ud]) {
                    ...
                    //         }
                    //     }
                    //     alert("2:"+JSON.stringify(form_pm_obj))
                    // }
...
function initTableList(myId) {
    // alert("colList"+JSON.stringify(colList))
    var myTabId = "list_grid";
    // alert("myId",myId)
    $("#list_grid,#list_gridPager").show();
    // $('.list_grid_tab').resize(function () {
    //     $('#'+myTabId).setGridWidth($('.list_grid_tab').width() * 1);
    // });
    // alert("参数："+JSON.stringify(getSearchParams()));
...
function getSearchCondition() {
    //参与搜索的字段
    var fieldKeys = [];
    // alert("fieldList:"+JSON.stringify(fieldList));
...
function getQuickQueryConfig() {
    ...
    $.each($("#queryForm").find(".form-group"), function () {
        ...
        var con = {
            field_key: field,
            con: condition
        }; //value:value,end_val:end_val,start_val:start_val,unit:unit});
        if (!condition) { //日期类型
	...
	$("#queryForm").find("div[condition=QUOTE] input[name=temp]").each(function () {
		...
		if (quoteStr) {
			...
			if (wordbookId && fieldKey) {
				...
				var tempMagicSuggest = $(this).magicSuggest({
...
if(form_pm_obj){
	for (var pd in form_pm_obj) {
		if (!postData[pd]) {
			postData[pd]=form_pm_obj[pd];
		}
	}
}*/
/*
//改为用户搜索分类加载之后进行（使默认搜索分类起作用）
...
function getSearchParams() {
    ...
    // alert("form_pm_obj:"+JSON.stringify(form_pm_obj))
    if (form_pm_obj) {
        for (var pd in form_pm_obj) {
            if (form_pm_obj[pd]) {
    ...
    // alert("form_db:"+JSON.stringify(form_db));
    if (form_db) {
        for (var pd in form_db) {
            if (form_db[pd]) {
                data[pd] = form_db[pd];
                ...
                if ($("#yb_template").val() == "1") {
                    ...
                    if (openMenuMark) {
                        ...
                        if (open_window_width && open_window_width > 0 && open_window_width > open_window_default_width) {
                            ...
                            // $("#table_block").html('<iframe id="f_'+tableId+'" name="content_iframe" class="content_iframe_s" src="'+ url+'" width="100%" height="100%" frameborder="0"></iframe>');
                            //showRightPopGetWidth("fb_btn_panel", open_window_default_width + "px");
                            //  $("#table_block").html('<iframe name="content_iframe" class="content_iframe_s" src="' + url + '" width="100%" height="' + iframeHeight + 'px" frameborder="0"></iframe>');
                        }
                    }
...
//     $("#"+divId).css({width:divWidth,right:-(divWidth)});
...
function openTableSetting() {
    ...
    // var url = '${ctx}/new/menu/setting.jsp?tableId=' + tableId + '&menuId=' + cur_menuId;
    var url = './setting.html?tableId=' + tableId + '&menuId=' + cur_menuId;
    // alert(tableId+"  2: "+cur_menuId);
    if (tableTeamId) { //表格组
        // url = '${ctx}/new/menu/setting.jsp?tableId=' + tableId + '&menuId=' + cur_menuId + '&type=TABLE_TEAM_TABLE&tableTeamMark=主表&tableTeamId=' + tableTeamId;
