...
$(document).ready(function(){$('.aspNetHidden').hide();document.onselectstart=function(){return false;}
...
function BeautifulGreetings(){var now=new Date();var hour=now.getHours();if(hour<3){return("夜深了,早点休息吧！")}
...
function IsEditdata(id){var isOK=true;if(id==undefined||id==""){isOK=false;showTipsMsg("很抱歉，您当前未选中任何一行！",4000,3);}else if(id.split(",").length>1){isOK=false;showTipsMsg("很抱歉，一次只能选择一条记录！",4000,3);}
return isOK;}
function IsDelData(id){var isOK=true;if(id==undefined||id==""){isOK=false;showTipsMsg("很抱歉，您当前未选中任何一行！",4000,3);}
return isOK;}
function IsChecked(id){var isOK=true;if(id==undefined||id==""){isOK=false;showTipsMsg("您当前未选中任何一行！",4000,3);}
return isOK;}
function IsNullOrEmpty(str){var isOK=true;if(str==undefined||str==""){isOK=false;}
return isOK;}
function delConfig(url,parm,count){DeleteConfig(url,parm,"注：删除操作不可恢复，您确定要继续么？",count);}
function DeleteConfig(url,parm,Msg,count){if(count==undefined){count=1;}
showConfirmMsg(Msg,function(r){if(r){getAjax(url,parm,function(rs){if(rs.toLocaleLowerCase()=='true'){showTipsMsg("成功删除 "+count+" 笔记录。",2000,4);windowload();}else if(rs.toLocaleLowerCase()=='false'){showTipsMsg("删除失败，请稍后重试",4000,5);}else{showTipsMsg(rs,4000,3);}});}});}
...
function GetQuery(key){var search=location.search.slice(1);var arr=search.split("&");for(var i=0;i<arr.length;i++){var ar=arr[i].split("=");if(ar[0]==key){return ar[1];}}
return"";}
function changeURLPar(destiny,par,par_value){var pattern=par+'=([^&]*)';var replaceText=par+'='+par_value;if(destiny.match(pattern)){var tmp='/\\'+par+'=[^&]*/';tmp=destiny.replace(eval(tmp),replaceText);return(tmp);}
else{if(destiny.match('[\?]')){return destiny+'&'+replaceText;}
...
function IsNumberbox(obj){$("#"+obj).bind("contextmenu",function(){return false;});$("#"+obj).css('ime-mode','disabled');$("#"+obj).bind("keydown",function(e){var key=window.event?e.keyCode:e.which;if(isFullStop(key)){return $(this).val().indexOf('.')<0;}
return(isSpecialKey(key))||((isNumber(key)&&!e.shiftKey));});function isNumber(key){return key>=48&&key<=57}
...
function iframeresize(height){if(height==undefined){height=0;}
...
$(".sub-menu div").click(function(){$('.sub-menu div').removeClass("selected")
$(this).addClass("selected");}).hover(function(){$(this).addClass("navHover");},function(){$(this).removeClass("navHover");});$('.layout-header-toggle').click(function(){window.parent.iframeleftHide();})
...
function ckbValueObj(e){var item_id='';var arry=new Array();arry=e.split('-');for(var i=0;i<arry.length-1;i++){item_id+=arry[i]+'-';}
...
function formatDate(date){var myyear=date.getFullYear();var mymonth=date.getMonth()+1;var myweekday=date.getDate();if(mymonth<10){mymonth="0"+mymonth;}
...
return fmt;}
function Quickseldate(type,BeginControID,EndControID){var begintime,endtime;if(type=="week"){endtime=getWeekEndDate();begintime=getWeekStartDate(endtime);}else if(type=="month"){endtime=getMonthEndDate();begintime=getMonthStartDate(endtime);}else if(type=="quarter"){begintime=getQuarterStartDate();endtime=getQuarterEndDate();}
...
function getQuarterStartMonth(){var quarterStartMonth=0;if(nowMonth<3){quarterStartMonth=0;}
...
(function(menu){jQuery.fn.contextmenu=function(options){var defaults={offsetX:2,offsetY:2,items:[],action:$.noop()};var opt=menu.extend(true,defaults,options);function create(e){var m=menu('<ul class="simple-contextmenu"></ul>').appendTo(document.body);menu.each(opt.items,function(i,item){if(item){if(item.type=="split"){menu("<div class='m-split'></div>").appendTo(m);return;}
var row=menu('<li><a href="javascript:void(0)"><span></span></a></li>').appendTo(m);item.icon?menu('<img src="'+item.icon+'">').insertBefore(row.find('span')):'';item.text?row.find('span').text(item.text):'';if(item.action){row.find('a').click(function(){item.action(e.target);});}}});return m;}
this.live('contextmenu',function(e){var m=create(e).show();var left=e.pageX+opt.offsetX,top=e.pageY+opt.offsetY,p={wh:menu(window).height(),ww:menu(window).width(),mh:m.height(),mw:m.width()}
top=(top+p.mh)>=p.wh?(top-=p.mh):top;left=(left+p.mw)>=p.ww?(left-=p.mw):left;m.css({zIndex:10000,left:left,top:top});$(document.body).live('contextmenu click',function(){m.remove();});return false;});return this;}})(jQuery);var GetRowIndex=-1;function PQLoadGrid(obj_ID,url,colM,sort,PageSize,topVisible){GetRowIndex=-1;var dataModel={location:"remote",sorting:"remote",paging:"remote",dataType:"JSON",method:"GET",curPage:1,rPP:PageSize,sortIndx:0,sortDir:"up",rPPOptions:[20,30,40,50,100,500,1000],getUrl:function(){var orderField=(this.sortIndx=="0")?"":sort[this.sortIndx];var sortDir=(this.sortDir=="up")?"desc":"asc";if(this.curPage==0){this.curPage=1;}
return{url:url,data:"pqGrid_PageIndex="+this.curPage+"&pqGrid_PageSize="+
this.rPP+"&pqGrid_OrderField="+orderField+"&pqGrid_OrderType="+sortDir+"&pqGrid_Sort="+escape(sort)};},getData:function(dataJSON){if(dataJSON==null&&dataJSON.totalRecords<=0){showTipsMsg("没有找到您要的相关数据！",5000,5);}
return{curPage:dataJSON.curPage,totalRecords:dataJSON.totalRecords,data:dataJSON.data};}}
if(!IsNullOrEmpty(topVisible)){topVisible=false;}
$(obj_ID).pqGrid({dataModel:dataModel,colModel:colM,editable:false,topVisible:topVisible,oddRowsHighlight:false,columnBorders:true,freezeCols:0,rowSelect:function(evt,ui){GetRowIndex=ui.rowIndxPage;}});pqGridResize(obj_ID,-52,-4);}
function PQLoadGridNoPage(obj_ID,url,colM,sort){GetRowIndex=-1;var dataModel={location:"remote",sorting:"remote",paging:"",dataType:"JSON",method:"GET",sortIndx:0,sortDir:"up",getUrl:function(){var orderField=(this.sortIndx=="0")?"":sort[this.sortIndx];var sortDir=(this.sortDir=="up")?"desc":"asc";return{url:url,data:"pqGrid_OrderField="+orderField+"&pqGrid_OrderType="+sortDir+"&pqGrid_Sort="+escape(sort)};},getData:function(dataJSON){if(dataJSON==null){showTipsMsg("没有找到您要的相关数据！",5000,5);}
...
function pqGridResize(obj,lose_height,lose_width){var grid_height=$(window).height()+lose_height;var grid_width=$(window).width()+lose_width;var grid1=$(obj).pqGrid({width:grid_width,height:grid_height});}
...
function GetPqGridRowValue(obj_ID,rowCode){if(GetRowIndex!=-1){var DM=$(obj_ID).pqGrid("option","dataModel");var data=DM.data;return data[GetRowIndex][rowCode];}
else{return null;}}
var IndetableRow_autocomplete=0;var scrollTopheight=0;function autocomplete(Objkey,width,height,data,callBack){if($('#'+Objkey).attr('readonly')=='readonly'){return false;}
...
$("#div_gridshow").find('tbody tr').click(function(){var value="";$(this).find('td').each(function(i){value+=$(this).text()+"≌";});if($('#'+Objkey).attr('readonly')=='readonly'){return false;}
if($('#'+Objkey).attr('disabled')=='disabled'){return false;}
callBack(value);$("#div_gridshow").hide();});$("#div_gridshow").find('tbody tr').hover(function(){$(this).addClass("selected");},function(){$(this).removeClass("selected");});document.onclick=function(e){var e=e?e:window.event;var tar=e.srcElement||e.target;if(tar.id!='div_gridshow'){if($(tar).attr("id")=='div_gridshow'||$(tar).attr("id")==Objkey){$("#div_gridshow").show();}else{$("#div_gridshow").hide();}}}}
function autocompletekeydown(Objkey,callBack){$("#"+Objkey).keydown(function(e){switch(e.keyCode){case 38:if(IndetableRow_autocomplete>0){IndetableRow_autocomplete--
...
function combotree(Objkey,width,height,data){$("#"+Objkey).bind("contextmenu",function(){return false;});$("#"+Objkey).css('ime-mode','disabled');$("#"+Objkey).keypress(function(e){return false;});if($('#'+Objkey).attr('readonly')=='readonly'){return false;}
if($('#'+Objkey).attr('disabled')=='disabled'){return false;}
...
function suolve(str){var sub_length=15;var temp1=str.replace(/[^\x00-\xff]/g,"**");var temp2=temp1.substring(0,sub_length);var x_length=temp2.split("\*").length-1;var hanzi_num=x_length/2;sub_length=sub_length-hanzi_num;var res=str.substring(0,sub_length);if(sub_length<str.length){var end=res+"…";}else{var end=res;}
return end;}
function moveTop(id){var selId=id;var selObj=document.getElementById(selId);if(selObj.options.length==0||selObj.options[0].selected){return false;}
...
function moveBottom(id){var selId=id;var selObj=document.getElementById(selId);if(selObj.options.length==0||selObj.options[selObj.options.length-1].selected){return false;}
...
function moveUp(id){var selId=id;var selObj=document.getElementById(selId);if(selObj.options.length==0||selObj.options[0].selected){return false;}
...
function moveDown(id){var selId=id;var selObj=document.getElementById(selId);if(selObj.options.length==0||selObj.options[selObj.options.length-1].selected){return false;}
...
function selAll(id){var selId=id;var selObj=document.getElementById(selId);if(typeof selObj=="undefined"||selObj==null||selObj==false){return false;}
