		...
		var schedulerConfig={
			...
			timeKeyAM:{"1":"01","2":"02","3":"03","4":"04","5":"05","6":"06","7":"07","8":"08","9":"09","10":"10","11":"11","12":"00"},
			timeKeyPM:{"1":"13","2":"14","3":"15","4":"16","5":"17","6":"18","7":"19","8":"20","9":"21","10":"22","11":"23","12":"12"},
			dayOfWeekNames:["周日", "周一", "周二", "周三", "周四", "周五", "周六"],
			movieData:[{id: 1,text: "His Girl Friday",director: "Howard Hawks",year: 1940,image: "../../../../images/movies/HisGirlFriday.jpg",duration: 92,color: "#cb6bb2"}],
			...
			dateCellTemplate:function(cellData, index, container) {
				...
				var aboutDate={"startWeekDate":beginWeek,"endWeekDate":endWeek,"startMonthDate":beginMonth,"endMonthDate":endMonth};
				if(format){
				...
				var params={"startDate":aboutDate.startWeekDateStr,"endDate":aboutDate.endWeekDateStr};
				...
				for(var i=0;i<leanderData.length;i++){
					...
					if(db.root_id && db.root_name){
						...
						timeList.push({"startDate":new Date(db.start_time),"endDate":new Date(db.end_time)})
				...
				if(tableObj && tableObj.buttonList && tableObj.buttonList.length>0 && (taskSetting.add_btn || taskSetting.update_btn) ){
					...
					if(buttonObj){
						...
						var pmDb={form:params,table:data};
						pmDb['sys']={};
		...
		function testData(){
			var theatreData = [{text: "Cinema Hall 1",id: 0}, {text: "Cinema Hall 2",id: 1}];
			var data = [{
				theatreId: 0,
				movieId: 3,
				price: 10,
			...
			return {leanderData:theatreData,taskData:data};
		}
		...
		function initScheduler(dataMap,loadMsg){
			...
			var scheduler = $("#"+schedulerid).dxScheduler({
				...
				firstDayOfWeek:1,
				views: [
					{type: "day",name: "日",intervalCount: 1,dateCellTemplate: schedulerConfig.dateCellTemplate},
					{type: "week",name: "周",dateCellTemplate: schedulerConfig.dateCellTemplate},
					{type: "month",name: "月",dateCellTemplate: schedulerConfig.dateCellTemplate}
				],
				...
				resources: [{fieldExpr: "root_id",dataSource: dataMap.leanderData},
					{fieldExpr: "movieId",dataSource:schedulerConfig.movieData,useColorAsDefault: true}
				],
					...
					if(data.name=="currentDate" || data.name=="currentView"){
						...
						if(data.value && data.value.length>0){
							...
							if(sData.groups && eData.groups && sData.groups.root_id==eData.groups.root_id){
								...
								lastSelectConfig={time:new Date().getTime(),data:sData}
							}
						}else{
				...
				onCellClick: function (e,b) {
					if(e.cellData.groups && e.cellData.groups.root_id){
						...
						if(curClickConfig && curClickConfig.time &&  (curLong-curClickConfig.time)<=300 && root_id==curClickConfig.root_id && startDate==curClickConfig.startDate && endDate==curClickConfig.endDate && validateLeanderIsApp(root_id,e.cellData.startDate,e.cellData.endDate)){
							...
							curClickConfig={time:curLong,startDate:startDate,endDate:endDate,root_id:root_id};
						}
					}
				},
