...
function queryInformationInfo(_this) {
    ...
    if (menu_type == "DEFINED") {
        if ("BI" == classify) {
            var bireportid = _this.attr("target_id");
            $.ajax({
                type: "get",
                async: false,
                data: {
                ...
                url: ctx + "/cloud/sys/defined/menu/reportshow/" + link_address,
                success: function (data) {
                    if (data.isOk) {
                        var str = data.queryStr;
                        if (str) {
                            str = str.replace("=", "@").replace("&", "@@");
                        }
                        ifaSrc = data.url + "&isLinks=" + str;
                    } else {
                        Notify("BI:" + data.msg, 'top-right', '5000', 'danger', 'fa-bolt', true);
                        flag = false;
                    }
                }
            });
        } else if ("REPORT" == classify) {
            $.ajax({
                url: ctx + "/cloud/sys/defined/menu/get",
                data: {
                    id: menu_id
                },
                type: "get",
                ...
                success: function (json) {
                    if (json.result == "SUCCESS") {
                        var definedMenu = json.obj;
                        if (definedMenu) {
                            var conditonConfig = {};
                            if (definedMenu.jsonField) {
                                conditonConfig = JSON.parse(definedMenu.jsonField);
                            }
                            sessionStorage.setItem("preConObj", conditonConfig.conObj);
                            sessionStorage.setItem("preSqlExpression", conditonConfig.sqlExpression);
                            tableListData(menu_id, conditonConfig.targetId, conditonConfig.targetName); //查询列表
                        } else {
                            Notify('自定义菜单不存在', 'top-right', '5000', 'danger', 'fa-bolt', true);
                        }
                    } else {
