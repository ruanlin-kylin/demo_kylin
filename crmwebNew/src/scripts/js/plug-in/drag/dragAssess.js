  ...
  $("#target").delegate(".component", "click", function(e){
    ...
    if(html_type=="INPUT" || html_type=="TEXTAREA"){
        ...
        $(".popover [name='max_date']").val(validate["max_date"]);
  		...
  		var options = [];
  		for(var o in formField["option"]){
  			options.push(formField["option"][o]+"/"+o);
  		}
  		$(".popover [name='option']").val(options.join("\n"));
        ...
        $(".popover [name='default_val']").val(formField["default_val"]);
        $(".popover [name='is_free']").attr("checked",formField["is_free"]=="0");				//能否自由输入
        ...
        if(wordbook){
			...
			$(".popover [name='params']").val(wordbook["params"]);
			$(".popover [name='renderer']").val(wordbook["renderer"]);
        ...
        //搜索框——初始化级联字段下拉菜单及搜索框
  		...
  		$(".popover [name='elem_height']").val(formField["elem_height"]);
  		if(formField["default_val"]){
  	  		$(".popover [name='default_val']").val(formField["default_val"]);
  		}else{
  	  		$(".popover [name='default_val']").val($active_component.find("[name='element']").html());
  		}
  	}
    ...
    $(".popover").delegate(".btn-info", "click", function(e){
    	...
    	formField["elem_width"]=getElemWidth();										//宽度
    	...
    	if(html_type=="INPUT" || html_type=="TEXTAREA"){
        	...
        	formField["validate"]=validate;
      		...
      		formField["default_val"]=$(".popover [name='default_val']").val();
        	formField["is_free"]=$(".popover [name='is_free']:checked").val()=="0"?"0":"1";	//能否自由输入
      		var wordbook={};
			...
			wordbook["params"]=$(".popover [name='params']").val();
			wordbook["renderer"]=$(".popover [name='renderer']").val();
      		var setting={};
      		var cascading=[];
      		...
      		formField["html_type"]=html_type;
      		formField["default_val"]=$(".popover [name='default_val']").val();
        	formField["elem_width"]=getElemWidth();
        	formField["elem_height"]=$(".popover [name='elem_height']").val();
      		$active_component.find("[name='element']").html(formField["default_val"]);
      		if(formField["elem_height"]){
      			$active_component.find("[name='element']").css("height",formField["elem_height"]);
      		}
      	}
    	...
    	var widthClass="col-xs-1 col-xs-2 col-xs-3 col-xs-4 col-xs-5 col-xs-6 col-xs-7 col-xs-8 col-xs-9 col-xs-10 col-xs-11 col-xs-12";
    	$active_component.removeClass(widthClass).addClass("col-xs-"+getElemWidth());
    });
    ...
    var widthMagic;
    //渲染宽度搜索框，可自定义输入
    function magicSuggestElemWidth(){
    	var defaultValue=[{text:"整行",value:"12"}];
        var elemWidth=$(".popover [name='elem_width']").val();
        if(elemWidth){
    	    if(elemWidth=="3"){
    	    	defaultValue=[{text:"1/4行",value:"3"}];
    	    }else if(elemWidth=="4"){
    	    	defaultValue=[{text:"1/3行",value:"4"}];
    	    }else if(elemWidth=="6"){
    	    	defaultValue=[{text:"半行",value:"6"}];
    	    }else if(elemWidth=="12"){
    	    	defaultValue=[{text:"整行",value:"12"}];
    	    }else{
    	    	defaultValue=[{text:elemWidth,value:elemWidth}];
    	    }
      	}
        widthMagic=$(".popover [name='elem_width']").magicSuggest({
    		data: [{text:"整行",value:"12"},{text:"半行",value:"6"},{text:"1/3行",value:"4"},{text:"1/4行",value:"3"}],
    ...
    function getElemWidth(){
  	  var widthList = widthMagic.getValue();
